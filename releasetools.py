...
def AddAssertions(info):
    ...
    for i in xrange(len(edify.script)):
        if " ||" in edify.script[i] and ("ro.product.device" in edify.script[i] or "ro.build.product" in edify.script[i]):
            edify.script[i] = edify.script[i].replace(" ||", ' || getprop("ro.build.product") == "ghost_retail" || getprop("ro.product.device") == "ghost" || getprop("ro.build.product") == "ghost_sprint" || getprop("ro.build.product") == "ghost_verizon" ||')
            return
